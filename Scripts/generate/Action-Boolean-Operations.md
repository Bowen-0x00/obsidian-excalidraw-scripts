...
function getRectPoints(el) {
    ...
    if (el.roundness) {
        ...
        const r = Math.min(w, h) * 0.15; // 近似默认圆角半径
        const poly = [];
        const steps = 8;
        // 左上角
...
function extractAbsolutePolygons(el) {
    // 1. 对于原生基础图形，直接使用完美的数学几何体，免疫所有渲染干扰
    ...
    // 2. 对于箭头、线条、手绘或你自定义的高级图形，提取底层渲染缓存
...
if (!accumulatedPolygons || accumulatedPolygons.length === 0) {
    new Notice("布尔运算失败：图形不相交或产生了非法结果！");
    return;
}
...
const newElementId = ea.addLine([[0, 0], [newWidth, newHeight]]);
